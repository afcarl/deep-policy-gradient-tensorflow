...
class Game(object):
  ...
  def runEpisode(self, sess, is_training=True):
    ...
    self.experience.add(state, 0, reward)
    state = next_state
    ...
    for t in xrange(self.config.NUM_STEPS):
      ...
      action = self.agent.act(
          sess, queried_state, self.env.action_space.sample(), self.epsilon)
      # Environment executes the action.
      next_state, reward, done, info = self.env.step(action)
      total_reward += reward

      # Add observation to experience.
      self.experience.add(state, action, reward)
    ...
    if is_training:
      # Update policy.
      for state, action, reward in self.experience.sample():
        self.agent.observe(sess, state, action, reward)
